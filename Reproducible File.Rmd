...
In this Rmarkdown file, we will show you how to reproduce our results for all figures in the main text of the paper. However, for most of the tasks, they are not feasible to run on a desktop machine, since each data point in the figure represents 200 independent replications and each replication takes several minutes to several hours to run depending on the dimension and the problem. Thus in this Rmarkdown file, we only provide the code with one single run.
...
### Make sure to put knockoff.R and MBHq.R are in the same directory with the Rmd file and set your workspace in this directory.

source('knockoff.R')
source('MBHq.R')
```

Some utilize function. analys function is to select the relevant features using Algorithm 1 in the paper. fdp_power is to calculate the fdp and power based on the selected features.
```{r}
...
The following code shows how to get the left hand side of Figure 2.
```{r, eval = FALSE}
...
data_save
```
Repeat the above step 500 times and the resulted swap probability for DS and MDS equals to $p_1/500$ and $p_2/500$ respectively. By varying sample size from $50$ to $5000$, we get the left hand side of Figure 2. 
...
The following code is the code for our DS and MDS algorithm. For different choice of $f$, we just simply change the construction of M. Here we show the optimal one and leave the other two choices in the annotation. We will use this code for further simulations in linear regression.

$\textbf{Run the following code before any other code for Figure 3-7.}$
...
data_save
```
In this code we only show how to get the results for $f_3$, by changing the construction of the mirror statistics in DS function, we are able to get the results for the other two construction, i.e. $f_1$ and $f_2$. Then repeat the same procedure 200 times and vary the correlation $\rho$ and the signal $\delta$, we are able to get the Figure 3 in the graph.
...
data_save
```
The code is rather similar to the code for Figure 3. We vary the number of DS replications ``num_split'' and fix other simualtion parameters.

## Figure 5-7: Linear regression

In this chunck, we consider reproducing Figure 5,6,7. We focus on one data point of the Figure 5 and the rest of the figures are almost the same. The data generating process are different across figures and we inlcude them in the annatation in the following code. For Figure 7, the real data design case, the data is available at
...
#fileloc = "your location of the data"
#data <- t(read.table("fileloc"))
...
In the paper, we apply DS and MDS to detect mutations in the HIV-1 that are associated with drug resistance. We copy the data cleaning and wrangling procedure from the analysis based on the knockoff: https://cran.r-project.org/web/packages/knockoff/vignettes/hiv.html. For a more detailed discussion on the data cleaning procuedre and data description, please see the link above.
...
We first do the data cleaning and get the reponse variables and the design matrix.
...
Here we include the code to reproduce Figure 8 and 9.
...
$\textbf{Make sure to run the following four chunks of code in order for the simulation code to work.}$
